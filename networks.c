...
// adds handle to buffer at pos, returns bytes added
int addHandleToBuff(uint8_t *buff, int pos, char *handle, uint8_t handleLen)
{
    buff[pos] = handleLen;
    memcpy(buff + pos + 1, handle, handleLen);
    return 1 + handleLen;
}

// reads handle from buffer and returns new byte offset/pos
int readHandle(uint8_t *buff, int pos, char *handle)
{
    uint8_t handleLen = buff[pos++];
    memcpy(handle, buff + pos, handleLen);
    handle[handleLen] = '\0';
    pos += handleLen;
    return pos;
}

// skips handles starting at byte offset. Returns new byte offset
int skipHandles(uint8_t *recvBuf, int byteOffset, int handlesToSkip)
{
    uint8_t handleLen;
    int i;
    for (i=0;i<handlesToSkip;i++)
    {
        handleLen = recvBuf[byteOffset++];
        byteOffset += handleLen;
    }
    return byteOffset;
}
